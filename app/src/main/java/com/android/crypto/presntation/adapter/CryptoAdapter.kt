    ...
    inner class CryptoViewHolder(val binding: ItemCryptoBinding) :
        RecyclerView.ViewHolder(binding.root) {
            ...
            fun bindData(coinsEntity: CoinsEntity){
                binding.tvItemCryptoRank.text = coinsEntity.rank.toString()
                binding.tvItemCryptoSymbol.text = coinsEntity.symbol
                binding.tvItemCryptoName.text = coinsEntity.name
                if (coinsEntity.is_active){
                    binding.tvItemCryptoIsActive.text = "active"
                    binding.tvItemCryptoIsActive.setTextColor(ContextCompat.getColor(binding.root.context,R.color.color_active))

                }else{
                    binding.tvItemCryptoIsActive.text = "inactive"
                    binding.tvItemCryptoIsActive.setTextColor(ContextCompat.getColor(binding.root.context,R.color.color_inactive))
